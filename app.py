...
# This will set the payload if the plane is already in the system. Or add the plane and payload if it is not already.

if plane in plane_models:
    max_payload = plane_payloads.pop(plane_models.index(plane))
    print("Your plane is set as a " + plane + ". With a max payload of " + str(max_payload) )
if plane not in plane_models:
    ...
    print("Your plane is set as a " + plane_models[-1] + ". With a payload of " + str(plane_payloads[-1] ) )
